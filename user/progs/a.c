...
int i =0;
    for (i = 0; i < NCHILD; ++i)
    {
      lprintf("The pid is :%d",pids[i]);
    }
    lprintf("The address is %p",(void *)pids);
  for (c = 0; c < NCHILD; ++c) {
    int slot;
    char msg[128];
    ...
    if ((c & 8) == 8)
      yield(-1);
    lprintf("Number of iteration %d", c);
    pid = wait(&slot);
    lprintf("Number of iteration haha %d", c);
    ...
    lprintf("fucking %p",(void *)pids);

    MAGIC_BREAK;
    if (pids[slot] != pid) {
      lprintf("The pids[slot] is %d", pids[slot]);
      lprintf("The pid is %d",pid);
      lprintf("The slot is %d", slot);
      MAGIC_BREAK;
